...
class Game{
    ...
    INLINE uint64_t getHash(bool tura) const {
        uint64_t h = 11816180607513830921ULL * red_pawn
        ^ 17816992196493881569ULL * black_pawn
        ^ 12329556594590792737ULL * red_king
        ^ 10419611012853566743ULL * black_king
...
public:
    ...
    INLINE Node find(uint64_t key) const;

    INLINE void insert(uint64_t key, uint8_t depth, int16_t score, uint16_t hashMove, uint8_t flag);
...
extern std::array<int16_t, 12> buff, saved;
...
template<bool Tura, bool King> int16_t dfs_pom(const Game &state, uint8_t depth, uint32_t nr, int16_t alpha, int16_t beta){
	...
	if constexpr (!Tura){
		...
		if(mv){
			if(UNLIKELY(depth == 0)){
                std::array<int16_t, 12> copy = buff;
				buff[11] = 0;
...
template<bool Tura> int16_t dfs(const Game &state, uint8_t depth, int16_t alpha, int16_t beta){
	...
	if(UNLIKELY((timeCount & 127) == 0)){
        ...
        if(UNLIKELY(t1 - t0 > 97000)){
            //int ms = (t1 - t0)/1000;
            //std::cerr << "Time: " << ms << " ms, MAX_DEPTH = "<<(int)MAX_DEPTH-1<<"\n";
			stop_iterating = true;
			return 0;
		}
	}
	uint64_t hashKey;
	if constexpr(!Tura) hashKey = state.getHash(false);
	else hashKey = state.getHash(true);
	Node node = tt.find(hashKey);
	int16_t origAlpha = alpha;
	int16_t origBeta  = beta;
	uint16_t hashMove = 0;
	uint8_t rem = MAX_DEPTH - depth;
    /* format hashmove and killerMove:
	...
	* hashmove & 1   if equal 1 then king else pawn */
	if(node.key != 0){
		if(node.depth >= rem && depth){
            ...
            else if(node.flag == FLAG_LOWER) alpha = std::max(alpha, node.score);
            else beta = std::min(beta, node.score);
	...
	if constexpr(!Tura){
		...
		if(LIKELY(!stop_iterating)){
			...
			tt.insert(hashKey, rem, res, hashMove, flag);
		}
		return res;
	}
	else{
